...
#define TAG "me.direcciona.adwords.android"

using namespace v8;

static Persistent<Object> bindingCache;

static void Adwordsandroid_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Adwordsandroid.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::AdwordsandroidBindings::lookupGeneratedInit(
...
static void Adwordsandroid_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Adwordsandroid_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Adwordsandroid_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AdwordsandroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AdwordsandroidBinding = {
	"me.direcciona.adwords.android",
	Adwordsandroid_init,
	Adwordsandroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_me_direcciona_adwords_android_AdwordsandroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("me.direcciona.adwords.android", &AdwordsandroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::AdwordsandroidBindings::lookupGeneratedInit));
}
